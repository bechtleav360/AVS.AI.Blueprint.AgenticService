...
| `infrastructure/test_disk_cache_service.py` | `DiskCacheService` | `set`/`get` (string/dict/list keys, namespace isolation), `delete` (return values), `exists` (before/after set/delete), `clear` (namespace-scoped and full), TTL expiration via mocked `time.time`, `hash` consistency for string/list/dict inputs, `list_namespaces`, `list_values` (content, TTL-entry exclusion, limit), `get_stats` field presence, lifecycle, context-manager protocol, an unwritable cache directory failing with a message that names the path and the options (and keeping the OSError as the cause); `claim` set-if-absent (first wins, second refused, the loser does not overwrite, exactly one of five callers wins, keys and namespaces independent, dict keys hashed consistently, TTL recorded so readers see expiry, a logically expired claim taken over, no TTL never expires, and an unreachable cache failing open) |
| `infrastructure/test_redis_cache_service.py` | `RedisCacheService` | Backed by `fakeredis`. `claim` via `SET NX EX` (first wins, second refused, the loser does not overwrite, exactly one of five callers wins, the TTL is applied to the key, namespaces are independent, and an unreachable cache fails open). *The rest of this file's coverage predates this table and is not itemised here.* |
| `infrastructure/test_agent_scoped_cache.py` | `AgentScopedCache` | Per-agent isolation over one shared backend (same key and partition name kept apart, a neighbour's key reading as absent, the partition carrying the agent, `list_namespaces` stripped back, the default partition scoped, per-agent `delete` and `claim`); `clear` for one partition and for all of this agent's, never the neighbours'; the `close` refusal, the root-namespace refusal, not registering itself, stats naming the agent; and the six paths through a registry view (scoped for a namespace, raw at the root, one lens per view, different lenses per agent, named caches scoped, unknown name still raising) |
| `infrastructure/test_cache_backend_factory.py`, `infrastructure/test_cache_sharing_via_registry.py`, `infrastructure/test_redis_url_utils.py` | `CacheBackendFactory`, registry cache sharing, Redis URL helpers | Present but not itemised in this table. |
| `sessions/test_api_client.py` | `SessionsApiClient` | `on_startup` (valid config, missing config/base_url/api_key → ValueError), `on_shutdown` (closes client, no-op when uninitialised), `get_job_detail`/`start_job`/`complete_job`/`cancel_job` (URL shape, required headers, payload contents, uninitialised guard) |
| `sessions/test_key_provider.py` | `SessionKeyProvider` | `on_startup` (initialises cache, reads source/env_var from config, missing config → ValueError), `on_shutdown` (clears cache, no-op when uninitialised), `get_session_key` (env hit/miss, config source, vault → NotImplementedError, remote source via respx — happy path/caching/404/missing session_id/missing URL, unknown source, cache-hit bypass, result cached after fetch), `invalidate_cache` (default and per-session key removal, no-op when empty/uninitialised) |
