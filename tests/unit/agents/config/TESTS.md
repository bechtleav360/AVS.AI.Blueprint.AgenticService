...
| `test_namespace_views.py` | `Config` | `for_namespace` views (C5), the audited `settings` hatch, `resolved_settings` agreeing with `get()` key by key, `namespaces` and its C6 refusal on a view |
