...
| `api/eventing/test_sessions_startup_resilience.py` | `AppBuilder.build()` with `event_bus = "sessions"` | The lifespan completes and the root REST endpoints answer when the sessions service URL points at a closed port -- a background SSE consumer that cannot connect must not stop the application starting. Offline: the refused port is bound and released on loopback, so the failure is ECONNREFUSED rather than a timeout on every platform. Moved here from `tests/integration/` (#80), where nothing ran it
| `api/scheduling/test_scheduler.py` | `SchedulerBase`, `SchedulerTickHandler` | `_trigger_tick` calls `tick()` and returns `{"status": "triggered"}`; `scheduler_mode` reads both values case-insensitively, has no default so an absent, empty or missing key raises with both values named, rejects an unrecognised value and resolves once; `tick_topic` derives from `app_name` and the (possibly renamed) scheduler name, honours the `topic=` override, rejects a missing identity and a wildcard, rejects whitespace in an explicit topic and rewrites it in a derived one; `wire()` creates the tick handler in event mode only, is idempotent, registers the trigger route once in both modes, refuses event mode without a topic-carrying `event_bus` and accepts both broker transports; `on_startup` starts no timer in event mode, creates and starts `AsyncIOScheduler` on the declared crontab in in-process mode, and starts no second timer or route when called twice (#43); `on_shutdown` calls `shutdown(wait=True)` only when running and allows a later restart; the in-process tick claim -- the winner ticks and a loser does not, the key carries the scheduler and the minute, the claim carries the tick TTL and its own namespace, three replicas sharing a real `DiskCacheService` run one tick between them, a later slot is claimable again, no cache means every replica ticks and startup warns, the timer is wired to the claimed wrapper rather than to `tick`, and event mode claims nothing; `validate_crontab` accepting the five standard fields, stripping whitespace and rejecting empty, nonsense and six-field (apscheduler) expressions, with event-mode wiring failing on a bad crontab and in-process wiring not reaching the check; the tick handler declares its topic, outranks the default handler priority, claims a delivery under `nats_topic`/`dapr_topic`/`topic`, declines other or absent topics, ticks its scheduler without publishing an event, lets a failing tick reach the transport edge, and two schedulers register under distinct names in a real `Registry` |
| `api/scheduling/test_scheduler_event_mode.py` | `SchedulerBase` through `AppBuilder.build()` | What only a built application can show: the tick topic reaches the Dapr subscription document and a CloudEvent posted to `/events/<topic>` runs `tick()` exactly once; event mode starts no in-process timer; `POST /api/<scheduler>/trigger` is actually served (it used to be added to the router after `include_router` had copied it); in-process mode declares no tick topic and registers no eventing endpoint; a registered scheduler with no `scheduler_mode`, and event mode with no `event_bus`, each fail the build naming both values; and the pure-scheduler shapes -- publishing opted into, and nothing at all by default. Offline, and deterministic because `health_check_dapr = false` stops the Dapr health check probing for a sidecar: without it the namespace supervisor degrades the root agent and its paused transports refuse the tick. Moved here from `tests/integration/` (#80)
| `api/utilities/test_cache.py` | `CacheManagementApi` | Which cache each endpoint reaches (`?name=` selecting a named cache, the default when none is given, evict clearing only the one it names) and how it fails (503 with no cache registered at all, 404 for an unknown name, the 404 listing the registered names); and the statistics payload **against real backends** -- a real `DiskCacheService` reporting its four fields, a real `RedisCacheService` over `fakeredis` reporting a different seven, and a backend whose own stats call fails answering an empty payload rather than a 500 |
