...
2. **Outcome → terminal-state mapping — verified against svc-sessions.**
   svc-sessions `JobStatus` reaches COMPLETED, CANCELLED, and FAILED. The `/fail` endpoint
   (`RUNNING→FAILED`) has been live since 2026-06-24, so a handler-signalled failure is
   recorded as FAILED rather than a COMPLETED job carrying an error-shaped result. Mapping:

   | `process()` outcome | Base action | Terminal state |
   |---|---|---|
   ...
   | `ValueError`, any other `Exception` | `fail_job(error={"message":...,"code":...})` | FAILED |
   | returns; `failure_of` → `JobError` | `fail_job(error=...)` | FAILED |
   | returns; `failure_of` → `None` (default) | `complete_job(result=...)` | COMPLETED |

   The base **owns** the mapping (calls `cancel_job`/`complete_job`/`fail_job` itself); it only re-raises
...
| `test_value_error_fails_job` | AC-3 |
| `test_oserror_raises_retryable_leaves_pending` | AC-3 |
| `test_generic_exception_fails_job` | AC-3 |
...
- **`JobStatus.FAILED` is reachable** via the svc-sessions `/fail` endpoint (live 2026-06-24).
  analyser#13 AC-4 "marks job failed" is now satisfied directly: `SessionsJobHandler` routes an
  unrecoverable exception, and any result a subclass flags via `failure_of`, to `fail_job`.
  (Earlier revisions of this plan called FAILED "dead"; that held only before `/fail` shipped.)
