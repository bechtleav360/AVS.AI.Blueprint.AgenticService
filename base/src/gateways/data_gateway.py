...
class DataGatewayError(Exception):
    ...
    def __init__(self, message: str, status_code: Optional[int] = None, is_transient: bool = False):
...
class DataGatewayClient:
    ...
    async def get_asset(self, asset_id: str, traceparent: Optional[str] = None) -> AssetMetadata:
        """
        ...
        with tracer.start_as_current_span("data_gateway.health_check") as span:
            try:
                ...
                if response.status_code == 200:
                    ...
                    span.set_status(Status(StatusCode.ERROR, f"Health check failed: {response.status_code}"))
                    return {
                        "status": "unhealthy",
    ...
    async def get_asset_by_url(self, asset_url: str, traceparent: Optional[str] = None) -> AssetMetadata:
        """
